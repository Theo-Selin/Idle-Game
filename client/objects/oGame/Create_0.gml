...
// Simple save helper
function save_write() {
    var js = json_stringify(global.save);
    var f = file_text_open_write("save.json");
    file_text_write_string(f, js);
    file_text_close(f);
}
...
if (!variable_global_exists("___init_done")) {
	...
	global.item_data = {
	    ...
	    cloth: {
	        ...
	        icon: spr_cloth,
	        category: "Materials",
	        desc: "Simple fabric. Useful for basic protection.",
			sell_price: 3,
	        drops: [ { enemy: "oSlime", chance: 0.1, min: 1, max: 1 } ]
	    },
	    coin_copper: {
	        name: "Coin",
	    ...
	    cloth_armor: {
			...
			sell_price: 20,
			desc: "Simple armor.",
	        equip_stats: { damage: 0, defense: 1, hp: 0, hit: 0.00, crit: 0.00, crit_dmg: 0.00 },
	        stats: { armor: 1 },
	        anim_sprites: {
	            idle: { up: spr_cloth_idle_up, down: spr_cloth_idle_down, left: spr_cloth_idle_left, right: spr_cloth_idle_right },
	            walk: { up: spr_cloth_walk_up, down: spr_cloth_walk_down, left: spr_cloth_walk_left, right: spr_cloth_walk_right },
	            run:  { up: spr_cloth_run_up,  down: spr_cloth_run_down,  left: spr_cloth_run_left,  right: spr_cloth_run_right  },
    ...
    if (file_exists("save.json")) {
        ...
        if (is_struct(loaded) && !is_undefined(variable_struct_get(loaded, "version"))) {
            ...
            if (!variable_struct_exists(global.save, "activity") || !is_struct(variable_struct_get(global.save, "activity"))) {
            ...
            // Ensure known item keys
            var inv = variable_struct_get(global.save, "inventory");
            if (is_undefined(variable_struct_get(inv, "oak_log")))      variable_struct_set(inv, "oak_log", 0);
            if (is_undefined(variable_struct_get(inv, "coin_copper")))  variable_struct_set(inv, "coin_copper", 0);
            if (is_undefined(variable_struct_get(inv, "stick")))        variable_struct_set(inv, "stick", 0);
            if (is_undefined(variable_struct_get(inv, "cloth")))        variable_struct_set(inv, "cloth", 0);
            if (is_undefined(variable_struct_get(inv, "cloth_armor")))  variable_struct_set(inv, "cloth_armor", 0);

            // Optional migration from unix fields
            if (variable_struct_exists(global.save, "last_active_unix") || variable_struct_exists(global.save, "last_save_unix")) {
                ...
                if (variable_struct_exists(global.save, "last_active_unix")) variable_struct_remove(global.save, "last_active_unix");
                if (variable_struct_exists(global.save, "last_save_unix"))   variable_struct_remove(global.save, "last_save_unix");
                global.__save_dirty = true;
            }
        }
    }
    ...
    if (is_undefined(global.save.last_active_dt) || global.save.last_active_dt == 0) global.save.last_active_dt = now_dt;

    var dt_seconds  = max(0, date_second_span(global.save.last_active_dt, now_dt));
    var max_offline = 60 * 60 * 8;
    var sim_seconds = clamp(dt_seconds, 0, max_offline);

    // Prepare an empty report
    global.offline_report = { seconds: sim_seconds, activity_type: "idle", lines: [], total_items: 0, kills: 0 };

    if (sim_seconds > 0) {
        var act = variable_struct_get(global.save, "activity");
        if (!is_struct(act)) {
            act = { type: "idle", portal_id: global.current_portal, resource_id: "", per_sec: 0, enemy_kind: "", kills_per_sec: 0, loot_per_kill: {} };
    ...
    // Tell UI to show the offline popup once
...
__prev_tod         = -1;     // last pushed time-of-day
__push_epsilon     = 0.01;   // only push when change > 1%

// Place phase in the middle of the DAY plateau
var __a = dawn_secs / __cycle_len_s;
var __b = day_secs  / __cycle_len_s;
__phase = __a + (__b * 0.5);

/// Helper: cosine ease (0..1 → 0..1)
function __ease_cos01(t) { return 0.5 - 0.5 * dcos(t * 180); }
...
function __phase_to_tod(_p) {
    ...
    if (p < a) {
        var t = p / max(EPS_SMALL, a);  return __ease_cos01(t);
    }
    p -= a;
    if (p < b) return 1.0;
    ...
    p -= b;
    if (p < c) {
        var t2 = p / max(EPS_SMALL, c); return 1.0 - __ease_cos01(t2);
    }
    return 0.0;
}
...
// Initial push now + follow-up sync
game_set_time_of_day(1.0);
__sync_frames = 2;

// Night fairy follower handle (auto-managed by oGame.Step only)
global.fx_follow_flame    = noone;
global.fx_follow_layer    = "FX_Overlays";
